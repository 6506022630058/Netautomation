...
class HomeApp:
    def __init__(self, root):
        ...
        self.config_button.grid(row=3, column=0, padx=5, pady=5)

        # Terminal Button
        self.terminal_button = tk.Button(root, text="Terminal", command=self.open_terminal)
        self.terminal_button.grid(row=3, column=1, padx=5, pady=5)
    ...
    def open_config(self):
        ...
        config_window = ConfigApp(ip, username, password)
        config_window.run()  # Start the config application

class TerminalApp:
    def __init__(self, ip, username, password):
        self.root = tk.Tk()
        self.root.title("Packet Tracer-like SSH Terminal")
        self.ip = ip
