...
class SNMPApp:
    ...
    # Check if the status actually changed
      ...
      if not errorIndication and not errorStatus:
        ...
        # Update button color only if the status has changed
        if updated_status != current_status:
            print(updated_status,current_status,index)
            color = "green" if updated_status == 1 else "red"
            self.interface_buttons[index-1].config(bg=color)


    def on_closing(self):
        if self.asyncio_task:
            self.asyncio_task.cancel()  # Cancel any ongoing tasks
        self.root.destroy()

    def run(self):
        # Run the asyncio loop in a thread
        loop = asyncio.get_event_loop()
        self.asyncio_task = loop.create_task(self.fetch_interfaces())
